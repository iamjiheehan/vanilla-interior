...
var _default = /*#__PURE__*/function (_layout) {
  ...
  _createClass(_default, [{
    ...
    value: function executeScript() {
      ...
      var draggables = document.querySelectorAll('.design__drag-item');
      var containers = document.querySelectorAll('.design__drag-zone');
      var chosenItem = [];
      ...
      containers.forEach(function (container) {
        container.addEventListener('dragover', function (e) {
          ...
          var count = container.querySelectorAll('.design__drag-item').length;
          var isAlreadyInContainer = container.contains(draggable);
          if (!isAlreadyInContainer && afterElement == null && count < 3 && chosenItem.length < 3) {
            ...
            var item = container.insertBefore(draggable, afterElement);
            chosenItem.push(item);
          } else if (draggable.classList.contains('item') && isAlreadyInContainer) {
            chosenItem.push(draggable);
          }
          var altTexts = chosenItem.map(function (image) {
            return image.alt;
          });
